    ...
    public List<Vector3Int> FindPath(Vector3Int start, Vector3Int end, Func<Node, Node, PathCost> costFunction)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (Vector3Int offset in neighbors)
            {
                ...
                {
                    //Debug.Log($"Out of bounds: {node.Position + offset}");
                    continue;
                }
                Node neighbor = grid[node.Position + offset];
                if (closed.Contains(neighbor))
                {
                    //Debug.Log($"Already closed: {neighbor.Position}");
                    continue;
                }

                if (node.PreviousSet.Contains(neighbor.Position))
                {
                    //Debug.Log($"Previously visited: {neighbor.Position}");
                    continue;
                }

                PathCost pathCost = costFunction(node, neighbor);
                if (!pathCost.traversable)
                {
                    //Debug.Log($"Not traversable: {neighbor.Position}");
                    continue;
                }

                // Additional debug for stairs
                if (pathCost.isStairs)
                {
                    //Debug.Log($"Stairs detected: {neighbor.Position}");
                    int xDir = Mathf.Clamp(offset.x, -1, 1);
                    int zDir = Mathf.Clamp(offset.z, -1, 1);
                    Vector3Int verticalOffset = new(0, offset.y, 0);
                    ...
                    {
                        //Debug.Log($"Stairs exclusion: {neighbor.Position}");
                        continue;
                    }
                }
                ...
                if (newCost < neighbor.Cost)
                {
                    ...
                    // Additional debug for cost updates
                    //Debug.Log($"Updated cost for: {neighbor.Position}, new cost: {newCost}");
